...
| `patches/jfsw/0004-psc-stick-sensitivity.patch` | **the console build only** (`-DAB_PSC`, `target_psc`): the joystick axes default to a scale of 0.20 (13107) instead of 1.0 - the virtual pad turns the D-pad into the left stick at full deflection, and at 1.0 that turned far too fast (the owner, 2026-09-25, from `20260105-2`). A `sw.cfg` saved before keeps its own value (the menu: Options -> Joystick setup) |
